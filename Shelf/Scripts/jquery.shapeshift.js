...
(function() {
  ...
  (function($, window, document) {
    ...
    //custom
	var shelf_orig_pos, shelf_new_pos,		 $clone, $container, $placeholder, $selected, active_class, clone_class, current_container_class, delete_clone, drag_clone, drag_rate, drag_timeout, dragged_class, options, original_container_class, placeholder_class, 		previous_container_class, selected_offset_x, selected_offset_y,
        _this = this;
        options = this.options;
        $container = this.$container;
        ...
        if (options.enableDrag) {
          $container.children("." + active_class).filter(options.dragWhitelist).draggable({
            ...
            start: function(e, ui) {
	 //custom
	//shelf_orig_pos = e.target.dataset.order;
	shelf_orig_pos = new Array();
	shelf_new_pos = new Array();
	var shelf_children = $("#sortable").children();
	$.each(shelf_children, function() {
		if (this.id != null & this.id != '') {
			shelf_orig_pos.push(this.id);
		}
	});
	console.log(shelf_orig_pos);
	...
	$.each(shelf_children, function() {
		if (this.id != null & this.id != '') {
			shelf_new_pos.push(this.id);
		}
	});
	console.log(shelf_new_pos);
